...
class TopographicMap:
    ...
    def trailhead_rating(self, trailhead: Position) -> int:
        assert self.get(trailhead) == MIN_HEIGHT

        def peak_trails(trail: Trail) -> Iterable[Trail]:
            pos = trail[-1]
            if self.get(pos) == MAX_HEIGHT:
                yield trail

            for n in self.adjacent_uphill(pos):
                yield from peak_trails(trail + (n,))

        return len(set(peak_trails((trailhead,))))
...
def main():
    ...
    total = sum(topo_map.trailhead_rating(t) for t in topo_map.trailheads())

    print(f"Sum of trailhead ratings: {total}")
