...
impl History {
    pub fn extrapolate_rev(&self) -> isize {
        #[tailcall]
        ...
        let reversed = self.history.iter().cloned().rev().collect::<Vec<_>>();
        let last = reversed.last().expect("History should not be empty");
        inner(*last, reversed)
    }
}
    ...
    let sum = reader
        ...
        .map(|(i, line)| {
            line.parse()
                ...
                .map(|h: History| h.extrapolate_rev())
        })
        .sum::<Result<isize>>()?;
